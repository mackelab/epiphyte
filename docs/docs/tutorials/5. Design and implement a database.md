...
generate mock data, define database tables, and instantiate a database
...
This notebook covers the basics of defining tables, namely:
...
The diagram below illustrates the general format of the mock dataset. 

* a. Movie stimulus, with ~30 seconds shown. 
* b. Movie feature annotations for each frame. Two time scales are involved in the movie playback: presentation timestamps (pts, time of the frame within the movie playback) and display timestamps (dts, real-world time at which the frame was shown).
* c. Spike times, concurrent with the movie playback. Times are clocked by the recording system and originally given in Unix time. 
* d. Local field potentials (LFP), concurrent with the movie playback. Same time scale as the spike times. 

![stimulus_features.png](5. Design and implement a database_files/stimulus_features.png)
...
The generated data is stored in `/epiphyte/data/mock_data/` and has the following structure: 


## 2. Identify the order of table population

The following diagram illustrates the overall organization of the database for the mock dataset: 

* **a.** Overview of the database ecosystem. Pink: Data formatting and refractoring are done prior to data entering the database. Green: Database interactions form the basis of analysis.
* **b.** Structure of the tables composing the database. 
* **c.** Dependencies for a sub-selection of tables from **b** (shown in the yellow box).

![database_schematic.png](5. Design and implement a database_files/database_schematic.png)

Since some tables pull information from other tables, we first need to decide on the order of table population.
...
The remaining tables, shown below, should be defined and instantiated according to the hierarchy, or "downstream" along the ERD.

![erd.png](5. Design and implement a database_files/erd.png)
...
The `ElectrodeData` table organizes the meta-data of the implanted electrodes. 

Since this information is patient-specific and consistent for all sessions from a given patient, it's a relatively high-level table (e.g. has a longer downstream than upstream). 
...
The `MovieAnnotation` table organizes and stores the stimulus feature annotation. 

All stimulus features are structured as **start times, stop times, and content indicators (values)**. The "character 1" label in section 1 of this tutorial would be broken down into the following vectors:

```
start_times = [0., 0.12, 0.16, 0.20]
stop_times = [0.119, 0.159, 0.119, 0.32]
values = [1, 0, 1, 0]
```

Character 1 has two continuous appearances in the "film" -- the entirety of the first scene and the beginning of the second scene. The appearance spans the pts 0.0 to 0.12. These times determine the first `start_times` and `stop_times` entries, and determines the entry given to the value indicator, `1`. 

The "outdoors" label has a single occurrence and would have the following break-down: 
```
start_times = [0., 0.16, 0.28]
stop_times = [0.159, 0.279, 0.32]
values = [0, 1, 0]
```

Non-binary values, such as "scene id", would be represented as: 
```
start_times = [0., 0.12, 0.28]
stop_times = [0.119, 0.279, 0.32]
values = [1, 2, 3]
```

Prior to table population, annoations are converted from indicator functions, $a = [a_1, a_2, \ldots, a_s]$, where $a_i \in \{0, 1\}$ indicates the status of frame $i$ in the movie for $s$ frames to the format detailed above using the module `epiphyte.processing_labels`. 
...
The `SpikeData` table contains the spike train data for all units. 
    ...
    def _make_tuples(self, key):
        ...
        for i_pat, pat in enumerate(patient_ids):
            ...
            for i_sesh, sesh in enumerate(pat_sessions):
                ...
                assert len(spike_files) == len(unit_ids), "Number of units in ElectrodeData doesn't match number of spiking files."
...
The `LFPData` table contains the local field potential data, samples and timestamps. 

Since LFP data is usually quite large, the table population routine is more likely to experience disruptions. To mitigate data loss during the upload procedure, the population function is manually defined since DataJoint doesn't always handle partial upload calls. 
        ...
        for i_sesh, sesh in enumerate([pat_sessions]):
            ...
            for ds_file in path_ds_dir.iterdir():
                ...
                LFPData.insert1({
                    ...
                    'timestamps': ds_dict.item().get("ts"),
...
Depending on the experiment conditions and the duration of the stimulus, there may be interruptions to the stimulus presentation. In the case of a movie stimulus shown to intracranial patients, there could be clinical interruptions such as the administration of medication or even a seizure, causing a non-linear relationship between the presentation timestamps and the display timestamps/neural recording system time. Despite these possible imperfections, the recorded data may still be useful. In this case, it's necessary to align the annotations to the particular movie playback experienced by the experiment participant. 

The `PatientAlignedMovieAnnotations` pulls from the `MovieAnnotations` table and the `Sessions` table and automatically aligns all annotations to each patient's own watchlog. 
...
The `MovieSkips` contains annotation-formatted vectors tracking any and all moments in which movie playback was discontinuous, i.e. a participant skipped through a section. 
...
The `MoviePauses` contains annotation-formatted vectors tracking any and all moments in which movie playback was paused.
        ...
        for i_pat, pat in enumerate(patient_ids):
            ...
            for i_sesh, sesh in enumerate([pat_sessions]):
                ...
                self.insert1({'patient_id': pat, 
                            ...
                            'stop_times': np.array(stop)}, skip_duplicates=True)
...
Continue to *[Develop an analysis workflow](<6. Develop an analysis workflow.md>)*.
